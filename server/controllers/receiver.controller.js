...
const { Receiver, Address, User } = db;
...
// Get All
exports.getAllReceivers = (req, res) => {
  Receiver.findAll({
    attributes: {
      exclude: ["number_of_request", "number_of_received_deliveries", "vegan", "pescatarian", "vegetarain", "celiac_desease", "peanut_allergy",
       "dairy_allergy", "seafood_allergy", "egg_allergy", "kosher", "user_id", "updatedAt", "createdAt", "time_frame_end", "time_frame_start", ],
    },
    include: [{
      model: User,
      attributes: {
        exclude: ['RoleRoleId', "user_id", "password", "bio", "activity_status", "createdAt", "updatedAt" ],
      },
      include: [{
        model: Address
      }]
    ...
    if (receivers) {
      ...
      res.send({
        message: `Cannot get Receivers!`,
      });
    }
  })
    .catch((err) => {
      res.status(500).send({
        message: 'Error getting all receivers.',
      });
    });
};
